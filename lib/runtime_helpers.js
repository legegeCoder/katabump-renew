...
function buildBrowserLaunchOptions(proxyConfig) {
    ...
    if (proxyConfig) {
        ...
        // Chromium 不支持 SOCKS 代理认证，凭据仅对 http 代理有意义
        if (proxyConfig.scheme !== 'socks4' && proxyConfig.scheme !== 'socks5') {
...
// Chrome 网络层错误中属于"代理/传输不可用"的子集（net::ERR_*）。
// ERR_ABORTED / ERR_CERT_* 等不在其中：它们多源于脚本自身的中断或 TLS 拦截，换代理无济于事。
const PROXY_LEVEL_NAV_ERROR_CODES = new Set([
    'ERR_TIMED_OUT',
...
function extractChromeNetErrorCode(message) {
    const match = /net::(ERR_[A-Z0-9_]+)/.exec(String(message || ''));
    return match ? match[1] : null;
}

// 判定 Playwright 抛出的导航错误是否属于代理/网络层故障（应交由外层换代理重试，而非 FATAL）
function isProxyLevelNavigationError(error) {
    if (!error) return false;
    const message = typeof error === 'object' && error.message !== undefined ? error.message : error;
    const code = extractChromeNetErrorCode(message);
    return code !== null && PROXY_LEVEL_NAV_ERROR_CODES.has(code);
}

// Playwright 导航超时：page.goto / page.reload 已发出请求（call log 出现 "navigated to"），
// 但时限内未达到 waitUntil 状态——黑洞节点的另一类症状：隧道能建、响应数据流中断。
// 注意 waitForURL / locator 超时不在此列：它们无法区分网络故障与业务未跳转/未渲染。
function isNavigationTimeoutError(error) {
    if (!error || typeof error !== 'object' || !error.message) return false;
    if (!/Timeout \d+ms exceeded/.test(error.message)) return false;
    return /^(page|frame)\.(goto|reload|goBack|goForward|setContent):/.test(error.message);
}
